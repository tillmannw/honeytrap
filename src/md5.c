...
char *mem_md5sum(u_char *msg, u_int32_t size)
{
	...
	/* allocate memory for md5sum string */
	if(!(md5sum = (char *) calloc(32 + 1, 1))) {
		logmsg(LOG_ERR, 1, "Error - Can't allocate memory for md5 checksum string.\n");
		return(NULL);
	}

	ptr = md5sum;
	for(i = 0; i < 16; i++) {
		snprintf(ptr, 4, "%02x", digest[i]);
		ptr += 2;
	}
